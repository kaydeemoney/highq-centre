...
def student_full_project():
    ...
    if project_exist and continue_question != "none":
        ...
        if (latest_section== "objective"):
            ...
            """at this point we check whether we have reached the end of the section
                and change it to theory if so"""

            if present_question_id=="none":
                latest_section="theory"
                last_question_id=theory_questions.query.filter_by(project_id=project_id).first().question_id
            else:
            ...
            tracing_next_question_list = theory_questions.query.filter_by(project_id=project_id, question_id=last_question_id).one()
            present_question_id = tracing_next_question_list.next_id

            if present_question_id=="none":
                return "you have reached the end of this project"
            else:
                present_question_details = theory_questions.query.filter_by(question_id=present_question_id).first()
            return render_template('student_theory_page.html', project_id=project_id, 
        ...
        """here, next question cannot be none as the project just starts so we dont need to be bothered about
            its theory"""
